...
import type { LogDetail } from "./monthlyAttendanceTypes";

type OverviewLogDetailModalProps = {
...
function onTimeBadgeClass(label: string) {
  ...
  if (label === "Weekend") return "border-blue-200 bg-blue-50 text-blue-700";
  return "border-outline-variant bg-surface-container text-on-surface-variant";
}

export default function OverviewLogDetailModal({
  detail,
  onClose,
}: OverviewLogDetailModalProps) {
  if (!detail) return null;

  return (
    <DashboardModal
      title={detail.dateLabel}
      titleId="overview-log-title"
      size="sm"
      onClose={onClose}
      headerBadge={
        <span
    ...
    >
      <DashboardModalStatGrid>
        <DashboardModalStat icon={<LoginOutlinedIcon />} label="Login" value={detail.loginTime} />
        <DashboardModalStat icon={<LogoutOutlinedIcon />} label="Logout" value={detail.logoutTime} />
        <DashboardModalStat
        ...
        />
        <DashboardModalStat icon={<AccessTimeOutlinedIcon />} label="Worked" value={detail.totalWorked} />
        <DashboardModalStat
        ...
        />
        <DashboardModalStat icon={<EventAvailableOutlinedIcon />} label="Leave" value={detail.leaveStatus} />
